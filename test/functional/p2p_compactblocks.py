#!/usr/bin/env python3
# Copyright (c) 2016-2020 The thecoffeecoins Core developers
...
# TestP2PConn: A peer we use to send messages to thecoffeecoinsd, and store responses.
...
class CompactBlocksTest(thecoffeecoinsTestFramework):
    ...
    # This test actually causes thecoffeecoinsd to (reasonably!) disconnect us, so do this last.
    ...
    # thecoffeecoinsd's choice of nonce.
    ...
    # Test that thecoffeecoinsd requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for thecoffeecoinsd to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        version = test_node.cmpct_version
        node = self.nodes[0]
        # thecoffeecoinsd will not send blocktxn responses for blocks whose height is
