...
# Copyright (c) 2010-2020 The thecoffeecoins Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""thecoffeecoins test framework primitive and message structures

CBlock, CTransaction, CBlockHeader, CTxIn, CTxOut, etc....:
    data structures that should map to corresponding structures in
    thecoffeecoins/primitives
...
# Objects that map to thecoffeecoinsd objects, which can be serialized/deserialized


class CAddress:
    __slots__ = ("net", "ip", "nServices", "port", "time")

    # see https://github.com/thecoffeecoins/bips/blob/master/bip-0155.mediawiki
    NET_IPV4 = 1
...
class CBlockLocator:
    ...
    def serialize(self):
        r = b""
        r += struct.pack("<i", 0)  # thecoffeecoins Core ignores version field. Set it to 0.
...
class CTransaction:
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in thecoffeecoinsd
            if (flags != 0):
                self.vin = deser_vector(f, CTxIn)
...
class msg_version:
    ...
    def deserialize(self, f):
        ...
        # But, unconditionally check it to match behaviour in thecoffeecoinsd
        try:
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in thecoffeecoinsd indicates these should be deserialized as blocks
