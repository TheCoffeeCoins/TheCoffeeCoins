...
"""Test thecoffeecoinsd with different proxy configuration.

Test plan:
- Start thecoffeecoinsd's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on thecoffeecoinsd side:
...
class ProxyTest(thecoffeecoinsTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: thecoffeecoinsd's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
        ...
        if test_onion:
            addr = "thecoffeecoinsostk4e4re.onion:8333"
            self.log.debug("Test: outgoing onion connection through node for address {}".format(addr))
            node.addnode(addr, "onetry")
            cmd = proxies[2].queue.get()
            assert isinstance(cmd, Socks5Command)
            assert_equal(cmd.atyp, AddressType.DOMAINNAME)
            assert_equal(cmd.addr, b"thecoffeecoinsostk4e4re.onion")
