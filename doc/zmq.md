...
The thecoffeecoins Core daemon can be configured to act as a trusted "border
router", implementing the thecoffeecoins wire protocol and relay, making
...
The ZeroMQ feature in thecoffeecoins Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building thecoffeecoinsd:
...
For instance:
    ...
    $ thecoffeecoinsd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/thecoffeecoinsd.tx.raw \
...
These options can also be provided in thecoffeecoins.conf.
...
From the perspective of thecoffeecoinsd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into thecoffeecoinsd directly. Furthermore, no information is
...
using. thecoffeecoinsd appends an up-counting sequence number to each
