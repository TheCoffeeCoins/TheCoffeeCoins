...
This guide describes how to build thecoffeecoinsd, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build thecoffeecoins Core from source.
...
### 4. Clone thecoffeecoins repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the thecoffeecoins Core repository to a directory.
...
It is not necessary to build wallet functionality to run `thecoffeecoinsd` or  `thecoffeecoins-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/thecoffeecoins/thecoffeecoins/blob/master/doc/descriptors.md), `sqlite` is required.
...
thecoffeecoins Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/thecoffeecoins/thecoffeecoins/issues/7714).
...
You can deploy a `.dmg` containing the thecoffeecoins Core application using `make deploy`.
...
## Building thecoffeecoins Core

### 1. Configuration

There are many ways to configure thecoffeecoins Core, here are a few common examples:
...
Run the following in your terminal to compile thecoffeecoins Core:
...
## Running thecoffeecoins Core

thecoffeecoins Core should now be available at `./src/thecoffeecoinsd`.
If you compiled support for the GUI, it should be available at `./src/qt/thecoffeecoins-qt`.

The first time you run `thecoffeecoinsd` or `thecoffeecoins-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/thecoffeecoins/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/thecoffeecoins"

touch "/Users/${USER}/Library/Application Support/thecoffeecoins/thecoffeecoins.conf"

chmod 600 "/Users/${USER}/Library/Application Support/thecoffeecoins/thecoffeecoins.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/thecoffeecoins/debug.log
```

## Other commands:

```shell
./src/thecoffeecoinsd -daemon      # Starts the thecoffeecoins daemon.
./src/thecoffeecoins-cli --help    # Outputs a list of command-line options.
./src/thecoffeecoins-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/thecoffeecoins-qt -server # Starts the thecoffeecoins-qt server mode, allows thecoffeecoins-cli control
```
