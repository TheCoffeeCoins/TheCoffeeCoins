...
#include <config/thecoffeecoins-config.h> /* for USE_QRCODE */
#endif
...
void ReceiveRequestDialog::setInfo(const SendCoinsRecipient &_info)
{
    ...
    QString uri = GUIUtil::formatthecoffeecoinsURI(info);
...
{
    if (!model) return;
    ui->amount_content->setText(thecoffeecoinsUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), info.amount));
}

void ReceiveRequestDialog::on_btnCopyURI_clicked()
{
    GUIUtil::setClipboard(GUIUtil::formatthecoffeecoinsURI(info));
}
