...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a thecoffeecoins address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new thecoffeecoinsAddressEntryValidator(parent));
    widget->setCheckValidator(new thecoffeecoinsAddressCheckValidator(parent));
}
...
{
    // return if URI is not valid or is no thecoffeecoins: URI
    if(!uri.isValid() || uri.scheme() != QString("thecoffeecoins"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!thecoffeecoinsUnits::parse(thecoffeecoinsUnits::BTC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parsethecoffeecoinsURI(QString uri, SendCoinsRecipient *out)
{
    ...
    return parsethecoffeecoinsURI(uriInstance, out);
}
...
QString formatthecoffeecoinsURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("thecoffeecoins:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(thecoffeecoinsUnits::format(thecoffeecoinsUnits::BTC, info.amount, false, thecoffeecoinsUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", thecoffeecoins_CONF_FILENAME));
    ...
    /* Open thecoffeecoins.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "thecoffeecoins.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "thecoffeecoins (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("thecoffeecoins (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for thecoffeecoins*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "thecoffeecoins.desktop";
    return GetAutostartDir() / strprintf("thecoffeecoins-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a thecoffeecoins.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=thecoffeecoins\n";
        else
            optionFile << strprintf("Name=thecoffeecoins (%s)\n", chain);
