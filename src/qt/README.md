This directory contains the source code for the thecoffeecoins Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
#### thecoffeecoinsgui.(h/cpp)

- Represents the main window of the thecoffeecoins UI.
...
- ClientModel is used by the main application `thecoffeecoinsgui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a thecoffeecoins:... link from a browser).
...
* UI elements like thecoffeecoinsAmountField, which inherit from QWidget.
* `thecoffeecoinsstrings.cpp`: automatically generated
* `thecoffeecoinsunits.(h/cpp)`: BTC / mBTC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/thecoffeecoins_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `thecoffeecoins-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `thecoffeecoins-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `thecoffeecoins-qt`
9. You're all set! Start developing, building, and debugging the thecoffeecoins Core GUI
