...
{
    columns << QString() << QString() << tr("Date") << tr("Type") << tr("Label") << thecoffeecoinsUnits::getAmountColumnTitle(walletModel->getOptionsModel()->getDisplayUnit());
...
{
    columns[Amount] = thecoffeecoinsUnits::getAmountColumnTitle(walletModel->getOptionsModel()->getDisplayUnit());
...
QString TransactionTableModel::formatTxAmount(const TransactionRecord *wtx, bool showUnconfirmed, thecoffeecoinsUnits::SeparatorStyle separators) const
{
    QString str = thecoffeecoinsUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), wtx->credit + wtx->debit, false, separators);
    if(showUnconfirmed)
    {
...
QVariant TransactionTableModel::data(const QModelIndex &index, int role) const
{
    ...
    case Qt::DisplayRole:
        ...
        case Amount:
            return formatTxAmount(rec, true, thecoffeecoinsUnits::SeparatorStyle::ALWAYS);
        } // no default case, so the compiler can warn about missing cases
        assert(false);
    case Qt::EditRole:
    ...
    case TxPlainTextRole:
        {
            ...
            details.append(formatTxAmount(rec, false, thecoffeecoinsUnits::SeparatorStyle::NEVER));
            return details;
        }
    case ConfirmedRole:
    ...
    case FormattedAmountRole:
        ...
        return formatTxAmount(rec, false, thecoffeecoinsUnits::SeparatorStyle::NEVER);
    case StatusRole:
