...
const char * const thecoffeecoins_CONF_FILENAME = "thecoffeecoins.conf";
const char * const thecoffeecoins_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/thecoffeecoins/thecoffeecoins/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //thecoffeecoins-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", thecoffeecoins_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "thecoffeecoins";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\thecoffeecoins
    // macOS: ~/Library/Application Support/thecoffeecoins
    // Unix-like: ~/.thecoffeecoins
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "thecoffeecoins";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/thecoffeecoins";
#else
    // Unix-like
    return pathRet / ".thecoffeecoins";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", thecoffeecoins_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure thecoffeecoins Core copyright is not removed by accident
    if (copyright_devs.find("thecoffeecoins Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The thecoffeecoins Core developers";
    }
