...
static RPCHelpMan validateaddress()
{
    return RPCHelpMan{"validateaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The thecoffeecoins address to validate"},
                },
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
...
static RPCHelpMan verifymessage()
{
    return RPCHelpMan{"verifymessage",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The thecoffeecoins address to use for the signature."},
...
static RPCHelpMan echo(const std::string& name)
{
    return RPCHelpMan{name,
                ...
                "thecoffeecoins-cli and the GUI. There is no server-side difference.",
                {
    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = Assert(EnsureAnyNodeContext(request.context).init)->ipc()) {
                // Spawn a new thecoffeecoins-node process and call makeEcho to get a
                // client pointer to a interfaces::Echo instance running in
                // that process. This is just for testing. A slightly more
                // realistic test spawning a different executable instead of
                // the same executable would add a new thecoffeecoins-echo executable,
                // and spawn thecoffeecoins-echo below instead of thecoffeecoins-node. But
                // using thecoffeecoins-node avoids the need to build and install a
                // new executable just for this one test.
                auto init = ipc->spawnProcess("thecoffeecoins-node");
                echo = init->makeEcho();
                ipc->addCleanup(*echo, [init = init.release()] { delete init; });
            } else {
                // IPC support is not available because this is a thecoffeecoinsd
                // process not a thecoffeecoinsd-node process, so just create a local
